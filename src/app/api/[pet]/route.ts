...
export async function GET (request: NextRequest, { params }: { params: Promise<{ pet: PetTab }> }) {
  ...
  if (!pet) return jsonResponse({ error: true, data: null }, { status: 404, statusText: 'Not Found', headers })
  const searchParams = request.nextUrl.searchParams
  const limit = Number(searchParams.get('limit')) || 20
  const page = Number(searchParams.get('page'))
  const search = searchParams.get('search')
  const { data } = await fetchBreeds({ animal: pet.tab, page, limit, search: search })
  return jsonResponse({
    data,
    pagination: {
      limit,
