    ...
    <div className="w-56 h-80 rounded-md shadow-lg overflow-hidden relative group">
      <div className="z-[1]">
        <ImageWithFallback
          ...
          src={`https://cdn2.the${petType}api.com/images/${referenceImageId}.jpg`}
          fallback={`/${petType}_404.jpg`}
          alt={name}
          width={400}
          height={400}
          priority
        ...
        <div className="p-2">
          ...
          {weight && (
            <p className="text-sm text-black/70">Weight: {weight.metric} kg</p>
          )}
          {height && (
            <p className="text-sm text-black/70">Height: {height.metric} cm</p>
          )}
        </div>
      </div>
      <div className="absolute h-full w-full bottom-0 left-[-100%] bg-black/85 transition-all duration-200 ease-in-out group-hover:left-0 p-2 flex flex-col">
        ...
        <p className="text-white/85">
          {temperament || "Not available yet 😿"}.
        </p>
        <Link
          href={`/${petType}s/${id}`}
          className="text-white py-2 text-center bg-purple-500 rounded-md block mt-auto hover:bg-purple-800"
          prefetch={false}
        >
          Details
        </Link>
