...
export default function SearchResults() {
  ...
  const selectedTab: PetTab =
    (searchParams.get("tab") as PetTab) || PetTab.Cats;
  const selectedPet = PETS.find((pet) => pet.tab === selectedTab) || PETS[0];
  const search = searchParams.get("search") ?? null;
  const { data, isLoading, fetchNextPage, hasNextPage } = useInfiniteQuery({
    queryKey: [selectedPet.tab, search],
    queryFn: ({ pageParam }) =>
      fetchPets({ pageParam, search, petType: selectedPet.tab }),
    initialPageParam: 0,
  ...
  return (
    <div className="flex flex-wrap gap-10">
      ...
      {Boolean(flatedData?.length) &&
        flatedData?.map((pet) => (
          <PetCard key={pet.id} {...pet} petType={selectedPet.animal} />
        ))}
      {data && hasNextPage && (
        <>
